...
module.exports = {
    ...
    async detectQuestionFromElastic(intent, objEntity) {
        ...
        let answer = '';
        let defaultAnswer = '';
        let searchParams = await queryDb.getDefault_Answer(intent, params);
        if (searchParams.length) {
            let searchEntities = await queryDb.getAnswer(intent, objEntity);
    ...
    async getQuestion() {
        let data = await queryDb.getAll();
        let dataRes = [];

        for (let i = 0; i < data.length; i++) {
            let block = {};
            block.text = data[i].question;
            block.params = data[i].entities;
            block.intent = data[i].intent;
            block.entities = data[i].params
            dataRes.push(block);
        }
        return dataRes;
    },
    ...
    async deleteSamples(questions) {
        ...
        if (result.sent) {
            for(let i =0;i<dataDelete.length;i++){
                ...
                let resultDL = await queryDb.deleteDocument(dataDelete[i].text);
